...
| **DoS.6.R.1** | Accepted: a malformed pubkey at deploy time bricks the account, but this is a feature — it surfaces a deployment bug at construction time rather than at first transaction. The constructor explicitly validates `pubkey.len() == 897` (`lib.rs:67-69`). |
