...
| ID | Mitigation |
| --- | --- |
| **Spoof.1.R.1** | Falcon-512 is EUF-CMA secure under the lattice assumptions reviewed by NIST. Forging a signature without the seed is computationally infeasible. The verifier path is `__check_auth` → `FalconVerifier::verify_512` (`contracts/falcon-512-core/src/verify.rs:58`), which uses the upstream-vetted hash-to-point + NTT-based verification. |
