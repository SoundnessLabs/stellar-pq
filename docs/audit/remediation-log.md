...
| Last updated | 2026-08-24 (audit-firm finding VER-005 fixed: maintainability and documentation items) |
...
| **VER-005** | Ten maintainability/documentation items: `hash_to_point` range check debug-only, unframed `nonce\|\|message` absorption undocumented, instance-storage footprint undocumented, duplicated `logn` local, undocumented defense-in-depth pubkey check, `decode_pubkey` partial-write contract undocumented, over-broad visibility of `verify_raw_512` + `ntt.rs` primitives, recomputed inverse-NTT scaling constant, dead `acc_len` check, deprecated `Events::publish` | Veridise audit (#1290) | Warning (≈ Informational) | **Fixed** (all ten items; item 1 as a checked `false` return so `__check_auth` stays panic-free; item 5 retained as documented defense-in-depth; item 7 makes `verify_raw_512` private and narrows `ntt.rs` to `pub(crate)`/private; item 8 adds `FALCON_512_NI = 128` with a compile-time derivation assert; item 10 migrates to `FalconInit`/`FalconRotate` `#[contractevent]` types with an unchanged wire shape) | gnosed | 2026-08-18 | 2026-08-24 | _pending commit_ | [`findings/VER-005-maintainability-and-documentation.md`](findings/VER-005-maintainability-and-documentation.md) |
